...
class NofasKeluarXlsx(models.AbstractModel):
    _name = 'report.djbc_nofas_keluar_v2.report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        # Find records in the view based on the date range
        docs = self.env['djbc.nofas_keluar_v2'].search([
            ('tgl_dok', '>=', date_start),
            ('tgl_dok', '<=', date_end),
        ])
        ...
        # Table Headers
        headers = [
            'Jenis Dok', 'No Aju', 'Tgl Aju', 'No Daftar', 'Tgl Daftar', 'No Pengeluaran',
            'Tgl Pengeluaran', 'No B/L', 'Tgl B/L', 'No Container', 'Penerima',
            'Kode Barang', 'Nama Barang', 'Jumlah', 'Satuan', 'Nilai', 'Currency',
            'Warehouse'
        ]
